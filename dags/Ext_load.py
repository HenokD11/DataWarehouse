...
def extract_data(ti):
    ...
    loaded_df_name=data_extractor.extract_data(file_name='20181030_d1_0930_1000.csv',return_json=True)
...
def populate_table(ti):
    ...
    conn.insert_to_table(trajectory_file_name, 'trajectories',from_file=True)
    conn.insert_to_table(vehicle_file_name, 'vehicles',from_file=True)

def clear_memory(ti):
    trajectory_file_name = ti.xcom_pull(key="trajectory",task_ids='extract_from_file')
    vehicle_file_name = ti.xcom_pull(key="vehicle",task_ids='extract_from_file')

    os.remove(f'../temp/{trajectory_file_name}')
    os.remove(f'../temp/{vehicle_file_name}')
